...
function Home({navigation}) {
  ...
  useEffect(() => {
    const fetchWeatherData = async () => {
      try {
        ...
        const response = await fetch('https://api.open-meteo.com/v1/forecast?latitude=51.0501&longitude=-114.0853&current_weather=true&hourly=temperature_2m,wind_speed_10m&daily=temperature_2m_max,temperature_2m_min,sunrise,sunset&timezone=auto');
  ...
  const hourlyForecastData = weatherData && weatherData.hourly ? weatherData.hourly.time.map((time, index) => ({
    time: time,
    ...
    return (
      <View style={styles.day}>
        <Text style={styles.innerDay}>
          {item.day} High: {item.temperature_2m_max}° Low: {item.temperature_2m_min}°
        </Text>
      </View>
    );
  };
      ...
      <ScrollView>
        <View style={styles.container}>
          ...
          >
            ...
            <Image source={require('./assets/snowIcon.png')} style={{width: 50, height: 50, marginTop: 20}}
                />
  ...
  <Text style={styles.windIn}>
    {weatherData && weatherData.current_weather ? `${weatherData.current_weather.wind_speed} km/h` : 'Loading...'}
  </Text>
</View>
        ...
        />
            ...
            <Text style={{color:'white',
              justifyContent: 'center',fontSize:15}}>The information showed is from City of Calgary</Text>
          </ImageBackground>
        </View>
      </ScrollView>
    </SafeAreaView>
